...
| `home/.chezmoiscripts/darwin/run_once_after_10-runtime-managers.sh.tmpl` | (run script) | nvm and rustup via their upstream installers (downloaded to a file, then run); uv, bun and Go come from Homebrew and are only verified here. This and the darwin scripts below put Homebrew on PATH themselves — a fresh Mac has no Homebrew on PATH until `.zshrc` deploys |
