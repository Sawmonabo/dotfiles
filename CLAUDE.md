...
| `home/.chezmoiscripts/darwin/run_onchange_after_50-apps-and-extensions.sh.tmpl` | (run script) | macOS after-phase, best-effort: `[packages.darwin].cask` GUI apps and `packages.vscode_extensions` in one `brew bundle install --no-upgrade` (`cask` + `vscode` entries). After-phase because a hand-installed app at another version cannot be adopted by its cask and must not abort the apply; re-runs when `packages.toml` changes |
